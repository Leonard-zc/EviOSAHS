...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run EviOSAHS direct single-model OSAHS binary experiments.")
    ...
    parser.add_argument(
        "--pipeline-overlap",
        choices=["off", "same_model"],
        default="same_model",
        help="Enable ordered multithread same-model overlap across visual, reason, and final.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--vl-batch-size", type=int, default=2, help="Batch size for visual, reason, and final prompts.")
    parser.add_argument(
        "--clinical-source",
    ...
    parser.add_argument("--vl-model-path", default=None, help="Optional local path to Qwen2.5-VL model weights.")
    parser.add_argument(
        "--vl-device",
        default=None,
        help="Optional explicit device for the single shared VL runner, for example cuda:0.",
    )
...
def stage_extra(
    ...
    extra: dict[str, Any] = {
        ...
        "pipeline_mode": "same_model_multithread_ordered_pipeline"
        if args.pipeline_overlap == "same_model"
    ...
    if stage == "final" and paths is not None:
...
def generate_visual_batch(
    ...
    for prompt_batch in chunked(prompts, batch_size):
        try:
            responses.extend(
                vl_runner.generate_batch(
                    [image_path] * len(prompt_batch),
                    prompt_batch,
...
def generate_text_batch(
    ...
    *,
    ...
    for prompt_batch in chunked(prompts, batch_size):
        try:
            responses.extend(vl_runner.generate_text_batch(prompt_batch, system_prompt=system_prompt))
        except Exception:
            if len(prompt_batch) == 1:
                raise
            for prompt in prompt_batch:
                responses.append(vl_runner.generate_text(prompt, system_prompt=system_prompt))
    return responses
...
def build_visual_record(
    ...
    raw_responses = generate_visual_batch(image_path, prompts, vl_runner, batch_size)
...
def build_final_outputs_batch(
    ...
    responses = generate_text_batch(prompts, vl_runner, max(1, len(prompts)), system_prompt=FINAL_SYSTEM_PROMPT)
...
def run_streaming_pipeline(
    ...
    write_stage_state(
        ...
        visual_tracker,
        status="running",
        output_path=paths["visual_jsonl"],
        error_path=paths["visual_errors"],
        extra=stage_extra("visual", args, vl_runner.model_name, args.vl_device or "auto"),
    )
    write_stage_state(
        paths["reason_state"],
        reason_tracker,
        status="running",
        output_path=paths["reason_jsonl"],
        error_path=paths["reason_errors"],
        extra=stage_extra("reason", args, vl_runner.model_name, args.vl_device or "auto"),
    )
    write_stage_state(
        paths["final_state"],
        final_tracker,
        ...
        extra=stage_extra("final", args, vl_runner.model_name, args.vl_device or "auto", paths=paths),
    )
    ...
    for sample_index, sample in enumerate(dataset):
        ...
        if visual_record is None:
            try:
                visual_record = build_visual_record(sample, sample_index, image_dir, vl_runner, args.vl_batch_size, args)
                ...
                write_stage_state(
                    ...
                    error_path=paths["visual_errors"],
                    current_item=image_name,
                    extra={**stage_extra("visual", args, vl_runner.model_name, args.vl_device or "auto"), "last_error": error_record},
                )
        ...
        if reason_record is None:
            try:
                reason_record = build_reason_record(visual_record, vl_runner, args.vl_batch_size, args)
                ...
                write_stage_state(
                    ...
                    error_path=paths["reason_errors"],
                    current_item=image_name,
                    extra={**stage_extra("reason", args, vl_runner.model_name, args.vl_device or "auto"), "last_error": error_record},
                )
                if not args.continue_on_error:
                    visual_tracker.finish()
                    reason_tracker.finish()
        ...
        try:
            final_record, csv_row = build_final_outputs_batch([reason_record], vl_runner, args)[0]
            ...
            write_stage_state(
                ...
                error_path=paths["final_errors"],
                current_item=image_name,
                extra={**stage_extra("final", args, vl_runner.model_name, args.vl_device or "auto", paths=paths), "last_error": error_record},
            )
            if not args.continue_on_error:
                visual_tracker.finish()
                reason_tracker.finish()
    ...
    write_stage_state(
        paths["visual_state"],
        visual_tracker,
        status="completed_with_errors" if visual_tracker.errors else "completed",
        output_path=paths["visual_jsonl"],
        error_path=paths["visual_errors"],
        extra=stage_extra("visual", args, vl_runner.model_name, args.vl_device or "auto"),
    )
    write_stage_state(
        paths["reason_state"],
        reason_tracker,
        status="completed_with_errors" if reason_tracker.errors else "completed",
        output_path=paths["reason_jsonl"],
        error_path=paths["reason_errors"],
        extra=stage_extra("reason", args, vl_runner.model_name, args.vl_device or "auto"),
    )
    write_stage_state(
        paths["final_state"],
        final_tracker,
        status="completed_with_errors" if final_tracker.errors else "completed",
        output_path=paths["final_jsonl"],
        error_path=paths["final_errors"],
        extra=stage_extra("final", args, vl_runner.model_name, args.vl_device or "auto", paths=paths),
    )
...
def run_ordered_pipeline(
    ...
    def record_stage_error(stage: str, image_name: str, exc: Exception) -> None:
        ...
        error_path = {"visual": paths["visual_errors"], "reason": paths["reason_errors"], "final": paths["final_errors"]}[stage]
        tracker = {"visual": visual_tracker, "reason": reason_tracker, "final": final_tracker}[stage]
        state_path = {"visual": paths["visual_state"], "reason": paths["reason_state"], "final": paths["final_state"]}[stage]
        output_path = {"visual": paths["visual_jsonl"], "reason": paths["reason_jsonl"], "final": paths["final_jsonl"]}[stage]
        with status_lock:
            ...
            write_stage_state(
                ...
                current_item=image_name,
                extra={**stage_extra(stage, args, vl_runner.model_name, args.vl_device or "auto", paths=paths if stage == "final" else None), "last_error": error_record},
            )
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if reason_record is None:
                        try:
                            reason_record = build_reason_record(item["visual_record"], vl_runner, args.vl_batch_size, args)
                            with status_lock:
                                append_jsonl(reason_record, paths["reason_jsonl"])
                                with lookup_lock:
                                    reason_lookup[image_name] = reason_record
    ...
    def persist_final_success(final_record: dict[str, Any], csv_row: dict[str, Any]) -> None:
        with status_lock:
            ...
            write_stage_state(
                ...
                output_path=paths["final_jsonl"],
                error_path=paths["final_errors"],
                current_item=final_record["image_path"],
                extra=stage_extra("final", args, vl_runner.model_name, args.vl_device or "auto", paths=paths),
            )
    ...
    def final_worker() -> None:
        try:
            while True:
                ...
                while len(batch_items) < args.vl_batch_size:
                    try:
                ...
                try:
                    ...
                    try:
                        batch_outputs = build_final_outputs_batch(record_batch, vl_runner, args)
                        ...
                        for queued_item in batch_items:
                            ...
                            try:
                                final_record, csv_row = build_final_outputs_batch([queued_item["reason_record"]], vl_runner, args)[0]
                                persist_final_success(final_record, csv_row)
                            except Exception as exc:
    ...
    visual_thread = threading.Thread(target=visual_worker, name="eviosahs-direct-visual")
    reason_thread = threading.Thread(target=reason_worker, name="eviosahs-direct-reason")
    final_thread = threading.Thread(target=final_worker, name="eviosahs-direct-final")
    visual_thread.start()
    reason_thread.start()
    final_thread.start()
    visual_thread.join()
...
def main() -> None:
    ...
    try:
        if args.pipeline_overlap == "same_model":
            final_records = run_ordered_pipeline(dataset, args.image_dir, vl_runner, paths, args)
        else:
            final_records = run_streaming_pipeline(dataset, args.image_dir, vl_runner, paths, args)
